...
% for cases = 1:5
    ...
    indices = 1:1:10000;
    ind_train = randsample(indices,ceil(train_percent * size(Layer1,1)));
    ind_test = setdiff(indices,ind_train);

    Cov_train = Layer1(ind_train,:,:);
    Cov_test = Layer1(ind_test,:,:);
    ...
    % Reshape covariances
    Cov_train = reshape(Cov_train,[(size(Layer1,2))^2,size(Cov_train,1)])';
    Cov_test = reshape(Cov_test,[(size(Layer1,2))^2,size(Cov_test,1)])';
...
% save AccuracyLayer1LogEucl.mat accuracy